...
import { fetchSubscriptionStart, subscriptionDeleteStart } from "../../../../store/actions/SubscriptionAction";
...
const ManageSubscription = () => {
  ...
  const subscriptions = useSelector((state) => state.subscriptions.subscription);
  const subscriptionDelete = useSelector((state) => state.subscriptions.subscriptionDelete);
  ...
  useEffect(() => {
    if (!subscriptions.loading && Object.keys(subscriptions.data).length > 0) {
      subscriptions.data.total_subscriptions <= 0 ? setShow('create') : setShow('list');
    }
  }, [subscriptions]);
  ...
  const handleDelete = (subscription_id) => {
    dispatch(subscriptionDeleteStart({
                  ...
                  <div className="">
                    <div className="sub-add-btn">
                      {subscriptions.data.total_subscriptions < 2 ? (
                        <button className="settings-submit-btn" onClick={() => setShow('create')}>Add Subscription</button>
                      ) : null}
                    </div>
                    <div className="subscribe-grid">
                      {subscriptions.data.subscriptions.map(subscription => (
                        <div key={subscription.id} className="subscribe-card">
                          ...
                          <div className="subscribe__flex">
                            ...
                            <div className="subscribe-btn">
                              <Link to={`/edit-subscription/${subscription.id}`} className="out-btn">Edit</Link>
                              <button
                                ...
                                disabled={subscriptionDelete.loading && subscriptionDelete.data.subscription_id == subscription.subscription_id}
                              >
                                {subscriptionDelete.loading && subscriptionDelete.data.subscription_id == subscription.subscription_id ? "Loading" : "Delete"}
                              </button>
                            </div>
                          </div>
                        </div>
