...
import { Form, Button, Image, Modal, Tab, Row, Col, Nav, InputGroup } from "react-bootstrap";
import { Link } from "react-router-dom";
import { connect } from "react-redux";
import { translate, t } from "react-multi-lang";
import data from '@emoji-mart/data'
import Picker from '@emoji-mart/react'
import { communityAssetFilesUploadStart } from "../../store/actions/CommunityAction";

const ChatUploadModal = (props) => {
  const initialData = {
    fileType: "image",
    accepted: "image/*",
    typeText: "Images",
  ...
  const handleFileSelect = e => {
    ...
    if (e.target.files.length > 0) {
      ...
      props.dispatch(communityAssetFilesUploadStart({
  ...
  const removeMedia = chatAssetId => {
    setChatMessage({
      ...chatMessage,
      chatAssets: chatMessage.chatAssets.filter(asset => asset.community_asset_id !== chatAssetId),
    });
  }
  ...
  const handleSubmit = e => {
    ...
    if (!props.chatFilesUpload.buttonDisable) {
      props.handleMessageSubmit(chatMessage);
    }
  }
                        ...
                        <div className="upload-multiple-img-close">
                          <Button className="icon-close-img" onClick={e => {
                            window.confirm(t("asset_remove_msg"))
                              ? removeMedia(asset.community_asset_id)
                              : e.preventDefault();
                          }}>
                            <Image
                              className="close-icon"
...
const mapStateToPros = (state) => ({
  ...
  chatFilesUpload: state.community.communityFilesUpload,
});
