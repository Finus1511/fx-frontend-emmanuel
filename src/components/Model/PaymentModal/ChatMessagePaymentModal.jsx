...
import { chatMessagePaymentByWalletStart } from "../../../store/actions/ChatAction";
...
const ChatMessagePaymentModal = (props) => {
  ...
  const handleSubmit = () => {
    props.dispatch(
      chatMessagePaymentByWalletStart({
        to_user_id: props.modelUser.user_id,
  ...
  const handleValidation = (values) => {
    dispatch(couponCodeValidationStart({
      ...values,
      to_user_id: props.modelUser.user_id,
      platform: "chat-message-payments",
    }));
  };
  return (
    <div className="payment-modal-sec">
      <Modal
        ...
        onHide={props.closeChatPaymentModal}
        size="xl"
            ...
            <div class="pay-amount-sec">
              ...
              <div className="pay-modal-token-sec">
                <div className="pay-modal-token">
                  <p>{t("tokens")}</p>
                  <p>{props.modelUser.amount} {""} {t("tokens")}</p>
                </div>
                {couponDiscount &&
                  <div className="pay-modal-token">
                ...
                <div className="pay-modal-token">
                  <h5>{t("total_token")}</h5>
                  <h4>{couponDiscount ?
                    props.modelUser.amount - couponDiscount.coupon_applied_amount
                    :
                    props.modelUser.amount}</h4>
                </div>
              </div>
              <Button
                className="default-btn"
                ...
                disabled={
                  chatMessagePayWallet.ButtonDisable ||
                  wallet.buttonDisable ||
                  wallet.data?.user_wallet?.remaining <
                  props.modelUser.amount
                }
              >
                {chatMessagePayWallet.loadingButtonContent
                  ? chatMessagePayWallet.loadingButtonContent
                  : t("pay")}
              </Button>
            </div>
...
export default connect(
  mapStateToPros,
  mapDispatchToProps
)(translate(ChatMessagePaymentModal));