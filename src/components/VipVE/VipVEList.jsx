...
import { creatorVirtualBookingListStart, creatorVirtualExperienceListStart } from "../../store/actions/CreatorVipVEAction";
...
const VipVEList = () => {
  ...
  const creatorBooking = useSelector(state => state.creatorVipVE.creatorVirtualBookingList);
  const userVirtualExperienceCreatedList = useSelector(state => state.creatorVipVE.userVirtualExperienceCreatedList);
  const [listData, setListData] = useState(currentRoute == "/user-ve-vip-created-list" ? userVirtualExperienceCreatedList : creatorBooking);
  ...
  useEffect(() => {
    if (currentRoute == "/user-ve-vip-created-list") {
  ...
  useEffect(() => {
    setListData(currentRoute === "/user-ve-vip-created-list" ? userVirtualExperienceCreatedList : creatorBooking)
              ...
              <div className="admin-booking-list-info">
                ...
                >
                  <Image
                    ...
                    className="svg-clone"
                  />&nbsp;
                  Virtual Experience {currentRoute == "/user-ve-vip-created-list" ? "Creations" : "Bookings"}
                </a>
                <p className="text-muted f-2">
                  The list contains the virtual experience {currentRoute == "/user-ve-vip-created-list" ? "requests" : "bookings"} made by you.
                </p>
              </div>
            </Col>
          </Row>
        ...
        <Row>
          <Col md={12}>
            <div className="table-wrap">
              ...
              {currentRoute == "/user-ve-vip-created-list" ? <div className="admin-booking-table">
                      ...
                      <tbody>
                        {listData.data.virtual_experiences.map((creator, i) => (
                          <tr key={i}>
                            ...
                            <td>
                              <Link to={`/user-vip-virtual-details/${creator.unique_id}`}>
                                View
                              </Link >
                            </td>
                          </tr>
                      ...
                      <tbody>
                        {listData.data.virtual_experience_bookings.map((creator, i) => (
                          <tr key={i}>
                            ...
                            <td>{creator.status_formatted}</td>
                            <td>
                              <Link to={`/user-vip-virtual-details/${creator.virtual_experience_info.unique_id}`}>
                                View
                              </Link >
                            </td>
                          </tr>
...
export default withRouter(VipVEList);