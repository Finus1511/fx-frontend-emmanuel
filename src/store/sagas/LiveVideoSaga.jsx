...
function* singleLiveVideoUpdateAPI(action) {
  try {
    const response = yield api.postMethod(
      "live_videos_host_update",
      action.data
    );
    if (response.data.success) {
      yield put(updateSingleLiveVideoSuccess(response.data.data));
    } else {
      yield put(updateSingleLiveVideoFailure(response.data.error));
      const notificationMessage = getErrorNotificationMessage(
        response.data.error
      );
      // yield put(checkLogoutStatus(response.data));
    ...
    yield put(updateSingleLiveVideoFailure(error));
    const notificationMessage = getErrorNotificationMessage(error.message);
    yield put(createNotification(notificationMessage));
  }
}
