...
import {
  ...
  DIGITAL_PRODUCT_DOWNLOAD_START,
...
import {
  ...
  digitalProductDownloadSuccess,
  digitalProductDownloadFailure,
  digitalProductPaymentWalletSuccess,
  digitalProductPaymentWalletFailure,
...
function* userProductsSaveAPI(action) {
  try {
    ...
    if (response.data.success) {
      ...
      window.location.assign("/single-product/" + response.data.data.user_product.user_product_unique_id);
    } else {
...
function* digitalProductDownloadAPI(action) {
  try {
    const response = yield api.postMethod("view_product_file", action.data);

    if (response.data.success) {
      yield put(digitalProductDownloadSuccess(response.data.data));
      const notificationMessage = getSuccessNotificationMessage(
        response.data.message
      );
      yield put(createNotification(notificationMessage));
    } else {
      yield put(digitalProductDownloadFailure(response.data.error));
      const notificationMessage = getErrorNotificationMessage(
        response.data.error
      );
      yield put(checkLogoutStatus(response.data));
    ...
    yield put(digitalProductDownloadFailure(error));
    const notificationMessage = getErrorNotificationMessage(error.message);
    yield put(createNotification(notificationMessage));
  }
}

function* digitalProductPaymentWalletAPI(action) {
  try {
    const response = yield api.postMethod("digital_product_payment_by_wallet", action.data);

    if (response.data.success) {
      yield put(digitalProductPaymentWalletSuccess(response.data.data));
      const notificationMessage = getSuccessNotificationMessage(response.data.message);
      yield put(createNotification(notificationMessage));
    } else {
      yield put(digitalProductPaymentWalletFailure(response.data.error));
      const notificationMessage = getErrorNotificationMessage(response.data.error);
      yield put(checkLogoutStatus(response.data));
      yield put(createNotification(notificationMessage));
    }
  } catch (error) {
    yield put(digitalProductPaymentWalletFailure(error));
    const notificationMessage = getErrorNotificationMessage(error.message);
    yield put(createNotification(notificationMessage));
  }
}
...
export default function* pageSaga() {
  ...
  yield all([yield takeLatest(DIGITAL_PRODUCT_DOWNLOAD_START, digitalProductDownloadAPI)]);
  yield all([yield takeLatest(DIGITAL_PRODUCT_PAYMENT_WALLET_START, digitalProductPaymentWalletAPI)]);
