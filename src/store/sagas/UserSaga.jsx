...
import {
  ...
  hideFollowersSuccess,
  hideFollowersFailure,
...
function* hideFollowersAPI(action) {
  try {
    const response = yield api.postMethod("update_show_followings", action.data);

    if (response.data.success) {
      yield put(hideFollowersSuccess(response.data));
      const notificationMessage = getSuccessNotificationMessage(response.data.message);
      yield put(createNotification(notificationMessage));
    } else {
      yield put(hideFollowersFailure(response.data.error));
      const notificationMessage = getErrorNotificationMessage(response.data.error);
      yield put(createNotification(notificationMessage));
    }
  } catch (error) {
    yield put(hideFollowersFailure(error));
    const notificationMessage = getErrorNotificationMessage(error.message);
    yield put(createNotification(notificationMessage));
  }
}
...
export default function* pageSaga() {
  yield all([
    ...
    yield takeLatest(HIDE_FOLLOWERS_START, hideFollowersAPI),
  ]);
}